...
if not os.path.exists(cfg['save_folder'] + cfg['name']):
    os.mkdir(cfg['save_folder'] + cfg['name'])


def train():
    # Equisetum
    train_dataset = EquisetumDataset(cfg['training_set'], SSDAugmentation(cfg['min_dim'], cfg['mean']))
    val_dataset = EquisetumDataset(cfg['val_set'], SSDAugmentation(cfg['min_dim'], cfg['mean']))
    ...
    optimizer = optim.SGD(net.parameters(), lr=cfg['lr'], momentum=args.momentum,
    ...
    print('Training SSD on:', cfg['name'])
    ...
    if args.visdom:
        ...
        iter_val_plot = create_vis_plot(viz, 'Iteration', 'Val Loss', vis_title, vis_legend)

    train_data_loader = data.DataLoader(train_dataset, cfg['batch_size'],
                                        shuffle=True, num_workers=cfg['num_workers'],
                                        collate_fn=detection_collate,
                                        pin_memory=True)

    val_data_loader = data.DataLoader(val_dataset, 1,
                                      shuffle=True, num_workers=cfg['num_workers'],
                                      collate_fn=detection_collate,
                                      pin_memory=True)
        ...
        if iteration % 100 == 0 and iteration != 0:
            ...
            if args.visdom:
                viz.line(
                    ...
                    win=iter_val_plot,
                    update='append' if iteration > 100 else None
                )

        if args.visdom:
            update_vis_plot(viz, iteration, loss_l.data, loss_c.data,
                            iter_plot, epoch_plot, 'append')

        if iteration != 0 and iteration % 200 == 0:
            print('Saving state, iter:', iteration)
            torch.save(ssd_net.state_dict(),
                       cfg['save_folder'] + cfg['name'] + '/' + 'ssd300_' + repr(iteration) + '.pth')
...
def adjust_learning_rate(optimizer, gamma, step):
    """Sets the learning rate to the initial LR decayed by 10 at every
        specified step
    # Adapted from PyTorh Imagenet example:
